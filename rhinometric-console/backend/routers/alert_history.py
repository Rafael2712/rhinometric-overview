...
def _apply_retention_and_exclusion(query, db: Session, time_range: str = None):
    ...
    # Includes: webhook alerts (category=external-services), and
    # platform policy alerts (entity_type=service, alert_name=policy:...).
    _ALLOWED_ENTITY_TYPES = {"external-services", "service"}
    query = query.filter(
        or_(
            ...
            AlertEvent.alert_name.like("policy:%"),
