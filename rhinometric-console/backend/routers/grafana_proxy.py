...
def _validate_grafana_token(jwt_token: str):
    """Validate a Keycloak JWT token and return (username, user) or raise."""
    from core.oidc import validate_keycloak_token, extract_user_info
    from database import SessionLocal

    claims = validate_keycloak_token(jwt_token)
...
async def grafana_render_proxy(
    ...
    # Verify user via Keycloak
    try:
        current_user = _validate_grafana_token(jwt_token)
    except Exception:
        raise HTTPException(status_code=401, detail="Invalid token")
...
async def grafana_proxy(
    ...
    # Decode token via Keycloak and get user
    try:
        current_user = _validate_grafana_token(jwt_token)
    except Exception:
