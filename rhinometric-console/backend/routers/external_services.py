...
from models.external_service_check import ExternalServiceCheck
from services.license_validator import validate_service_creation as _validate_service_limit, get_plan_check_interval as _get_plan_check_interval, LicenseLimitError as _LicenseLimitError
...
async def import_external_services(
    ...
    # -- LICENSING: pre-flight limit check for non-dry-run imports --
    if not dry_run:
        try:
            _validate_service_limit(db)
        except _LicenseLimitError as _lic_err:
            raise HTTPException(
...
def bulk_create_http_services(
    ...
    # -- LICENSING: pre-flight limit check for non-dry-run bulk creates --
    if not dry_run:
        try:
            _validate_service_limit(db)
        except _LicenseLimitError as _lic_err:
...
def create_external_service(
    ...
    # -- LICENSING: enforce monitor limit for current plan --
    try:
        _validate_service_limit(db)
    except _LicenseLimitError as _lic_err:
        raise HTTPException(
    ...
    # -- LICENSING: assign check interval from plan --
    payload.check_interval_seconds = _get_plan_check_interval(db)
