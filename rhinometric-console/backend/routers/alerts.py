...
async def get_alerts(
    ...
    All Source 1/2 alerts are synced to AlertEvent.
    CRITICAL alerts are automatically escalated to incidents with status="escalated"
    and are excluded from active alerts list (managed in Incidents module instead) (find-or-create)
    ...
    try:
        ...
        # =================================================================
        _active_statuses_set = ["firing", "acknowledged", "silenced"]  # NOTE: "escalated" excluded (managed in Incidents)
...
async def grafana_webhook(
    ...
    for alert in alerts:
        ...
        try:
            ...
            logger.warning("[Auto-Escalation] Failed for webhook alert (non-fatal): %s", _esc_err)
