...
async def generate_executive_report(db: Session, time_range: str) -> Dict[str, Any]:
    ...
    mon = services_summary.get("monitored_services", {})
    services_monitored = mon.get("total", services_summary.get("external_services", 0))
    services_healthy = mon.get("healthy", services_summary.get("healthy", 0))
    services_degraded = mon.get("degraded", services_summary.get("degraded", 0))
    services_down = mon.get("down", services_summary.get("down", 0))

    # Anomaly count for executive context
    exec_anomalies_count = 0
    try:
        from routers.anomalies import _fetch_and_normalize, _build_anomaly_groups  # type: ignore
        _anorm = await _fetch_and_normalize(time_range, 300)
        _agrps = _build_anomaly_groups(_anorm)
        exec_anomalies_count = len([g for g in _agrps if not _is_test_entity(g.get("entity_name")) and g.get("status") == "active"])
    except Exception:
        exec_anomalies_count = 0
    ...
    if incidents:
        ...
        if services_monitored > 0:
            summary_parts.append(f"{services_monitored} services are monitored ({services_healthy} healthy, {services_degraded} degraded, {services_down} down).")
        summary = " ".join(summary_parts)
    else:
        # No incidents — explain degraded/critical through other signals
        signals: List[str] = []
        if exec_anomalies_count > 0:
            signals.append(f"{exec_anomalies_count} active anomaly group(s) detected")
        if still_firing:
            firing_names = list({a.entity_name for a in still_firing if a.entity_name})[:3]
            signals.append(f"{len(still_firing)} critical alert(s) currently firing" + (f" ({', '.join(firing_names)})" if firing_names else ""))
        if services_down > 0:
            signals.append(f"{services_down} monitored service(s) currently down")
        elif services_degraded > 0:
            signals.append(f"{services_degraded} monitored service(s) in degraded state")
        if signals and status != "Operational":
            summary = (
                f"No formal incidents were recorded in the past {time_range}, but the platform status is {status}. "
                f"Degradation signals: {'; '.join(signals)}. "
                f"{services_monitored} services are monitored ({services_healthy} healthy, {services_degraded} degraded, {services_down} down). "
                "Immediate investigation of these signals is recommended."
            )
        elif services_monitored > 0:
            summary = (
                f"No incidents were recorded in the past {time_range}. Platform is {status} with {uptime}% uptime. "
                f"{services_monitored} services are monitored: {services_healthy} healthy, {services_degraded} degraded, {services_down} down."
            )
        else:
    ...
    return {
        ...
        "kpis": {
            ...
            "anomalies_count": exec_anomalies_count,
            "services_monitored": services_monitored,
            "healthy_count": services_healthy,
            "degraded_count": services_degraded,
            "down_count": services_down,
        },
...
async def generate_technical_report(db: Session, time_range: str) -> Dict[str, Any]:
    ...
    # Services data for technical report
    tech_services_summary = await get_services_summary(db)
    tech_mon = tech_services_summary.get("monitored_services", {})
    tech_plat = tech_services_summary.get("platform_components", {})

    # Services list from DB
    services_list: List[Dict[str, Any]] = []
    try:
        from models.external_service import ExternalService  # type: ignore
        enabled_svcs = db.query(ExternalService).filter(ExternalService.enabled == True).order_by(ExternalService.name).all()  # type: ignore
        services_list = [
            {
                "name": s.name,
                "status": (s.status.value if hasattr(s.status, "value") else str(s.status)),
                "url": getattr(s, "url", None),
                "last_checked_at": _to_iso(getattr(s, "last_checked_at", None)),
                "response_time_ms": getattr(s, "response_time_ms", None),
            }
            for s in enabled_svcs
            if not _is_test_entity(s.name)
        ]
    except Exception:
        services_list = []

    # Add report-generated event to timeline if empty
    if not timeline:
        timeline.append({
            "timestamp": now.isoformat(),
            "type": "report_generated",
            "entity": "Rhinometric Platform",
            "severity": "info",
            "status": "info",
        })

    # Latest signal events for lightweight timeline
    if len(timeline) <= 1 and (anomalies or alerts):
        if anomalies:
            latest_anomaly = sorted(anomalies, key=lambda a: a.get("last_seen") or "", reverse=True)
            if latest_anomaly and latest_anomaly[0].get("last_seen"):
                timeline.append({
                    "timestamp": latest_anomaly[0]["last_seen"],
                    "type": "anomaly_detected",
                    "entity": latest_anomaly[0].get("entity_name", "Unknown"),
                    "severity": latest_anomaly[0].get("severity") or "warning",
                    "status": "active",
                })
        if alerts:
            latest_alert = sorted(alerts_serialized, key=lambda a: a.get("last_seen") or "", reverse=True)
            if latest_alert and latest_alert[0].get("last_seen"):
                timeline.append({
                    "timestamp": latest_alert[0]["last_seen"],
                    "type": "alert_firing",
                    "entity": latest_alert[0].get("entity_name", "Unknown"),
                    "severity": latest_alert[0].get("severity") or "warning",
                    "status": latest_alert[0].get("last_status", "firing"),
                })
        timeline.sort(key=lambda x: x.get("timestamp") or "", reverse=True)

    tech_platform_status = _status_from_summary(str(tech_services_summary.get("overall_status", "OPERATIONAL")))
    active_alert_count = db.query(AlertEvent).filter(AlertEvent.status.in_(["firing", "escalated"])).count()
    ...
    return {
        ...
        "operational_snapshot": {
            "platform_status": tech_platform_status,
            "services_monitored": tech_mon.get("total", 0),
            "services_healthy": tech_mon.get("healthy", 0),
            "services_degraded": tech_mon.get("degraded", 0),
            "services_down": tech_mon.get("down", 0),
            "platform_components_total": tech_plat.get("total", 0),
            "platform_components_healthy": tech_plat.get("healthy", 0),
            "platform_components_down": tech_plat.get("down", 0),
