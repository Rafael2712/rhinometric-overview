...
# Import routers
from routers import auth, kpis, license, alerts, logs, traces, dashboards, settings as settings_router, users, grafana_proxy, correlation, external_services, anomalies
...
async def startup_event():
    ...
    try:
        with engine.connect() as conn:
            ...
            conn.execute(text("ALTER TABLE incidents ADD COLUMN IF NOT EXISTS ai_decision JSONB"))
            conn.commit()
        logger.info("incidents.tags and incidents.ai_decision columns ready")
    except Exception as e:
    ...
    try:
        ...
        try:
            with engine.connect() as conn:
                ...
                conn.commit()
            logger.info("alert_events.incident_id and alert_events.ai_decision columns ready")
    ...
    # Auto-create anomaly_ai_decisions table (AI Triage persistence)
    try:
        from models.anomaly_ai_decision import AnomalyAiDecision
        AnomalyAiDecision.__table__.create(bind=engine, checkfirst=True)
        logger.info("Anomaly AI decisions table ready")
    except Exception as e:
        logger.warning(f"Could not create anomaly_ai_decisions table: {e}")
...
app.include_router(assertions_router.router, prefix=f"{settings.API_PREFIX}", tags=["Assertions"])  # Assertions CRUD v1
