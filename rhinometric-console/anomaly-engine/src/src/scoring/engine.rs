...
use crate::scoring::{composite, confidence, evidence, prediction};
use crate::threshold::PredictionConfig;

/// Run the full scoring pipeline for a single service snapshot.
///
/// Pipeline: categorise → composite → confidence → severity → evidence → prediction → output
pub fn evaluate(snap: &ServiceSignalSnapshot, pred_cfg: &PredictionConfig) -> AnomalyOutput {
    ...
    // 8. Predictive risk layer (V1.5)
    let pred = prediction::predict(snap, pred_cfg);
    ...
    AnomalyOutput {
        ...
        trace_p95_latency_ms: snap.trace_p95_latency_ms,
        trace_error_rate: snap.trace_error_rate,
        trace_bottleneck_score: snap.trace_bottleneck_score,
        trace_slow_operations: snap.trace_slow_operations,
        predicted_risk_score: pred.predicted_risk_score,
        predicted_risk_level: pred.predicted_risk_level,
        predicted_horizon_minutes: pred.predicted_horizon_minutes,
        prediction_confidence: pred.prediction_confidence,
        prediction_reason_codes: pred.prediction_reason_codes,
...
mod tests {
    ...
    fn test_healthy_service() {
        let snap = base_snap();
        let output = evaluate(&snap, &pred_cfg());
    ...
    fn test_fingerprint_deterministic() {
        let snap = base_snap();
        let o1 = evaluate(&snap, &pred_cfg());
        let o2 = evaluate(&snap, &pred_cfg());
