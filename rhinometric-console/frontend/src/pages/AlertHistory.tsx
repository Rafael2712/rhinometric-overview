...
function StatCard({ label, value, icon: Icon, color }: {
  ...
  return (
    <div className="bg-surface border border-gray-200 rounded-lg p-4 flex items-center gap-3">
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <button onClick={exportCSV}
            className="flex items-center gap-2 px-3 py-2 bg-surface border border-gray-200 rounded-lg text-sm text-gray-600 hover:bg-gray-50 transition-colors">
      ...
      <div className="flex flex-wrap gap-3 items-center">
        ...
        <select value={timeRange} onChange={e => setTimeRange(e.target.value)}
          className="bg-surface border border-gray-200 text-gray-900 text-sm rounded-lg px-3 py-2">
      ...
      {isLoading && (
        <div className="flex items-center justify-center py-20">
          ...
          <span className="ml-3 text-gray-500">Loading alert history...</span>
        </div>
      )}
      ...
      {!isLoading && !error && events.length === 0 && (
        <div className="bg-surface border border-gray-200 rounded-lg p-12 text-center">
          <Clock className="mx-auto text-gray-500 mb-3" size={48} />
          <h3 className="text-lg font-semibold text-gray-600">No Alert Events</h3>
          <p className="text-sm text-gray-500 mt-1">
      ...
      {/* Desktop Table */}
      {!isLoading && !error && events.length > 0 && (
        <div className="hidden md:block bg-surface border border-gray-200 rounded-lg overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-200">
                  <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase">Alert</th>
                  ...
                  <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase">AI</th>
                  ...
                  <>
                    ...
                    >
                      <td className="px-4 py-3">
                        <div className="font-medium text-gray-900">{ev.alert_name}</div>
                        {ev.metric_name && (
                          <div className="text-xs text-gray-500 mt-0.5">{ev.metric_name}</div>
                        )}
                      </td>
                      <td className="px-4 py-3">
                        <div className="text-gray-600">{ev.entity_name || '—'}</div>
                      ...
                      <td className="px-4 py-3">
                        <span className={`font-mono text-sm ${
                          ...
                          ev.duration_seconds && ev.duration_seconds > 3600 ? 'text-amber-400' : 'text-gray-600'
                        }`}>
                          {ev.status === 'firing' ? (
                      ...
                      <td className="px-4 py-3 text-center">
                        {expandedRow === ev.id ? <ChevronUp size={16} className="text-gray-500" /> : <ChevronDown size={16} className="text-gray-500" />}
                      </td>
                    </tr>
                    {expandedRow === ev.id && (
                      <tr key={`${ev.id}-detail`} className="bg-gray-50/30">
                        <td colSpan={9} className="px-6 py-4">
                          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                            <div>
                              <span className="text-gray-500 block text-xs">Fingerprint</span>
                              <span className="text-gray-600 font-mono text-xs">{ev.fingerprint}</span>
                            </div>
                            <div>
                              <span className="text-gray-500 block text-xs">Source</span>
                              <span className="text-gray-600">{ev.source || 'alertmanager'}</span>
                            </div>
                            <div>
                              <span className="text-gray-500 block text-xs">Created At</span>
                              <span className="text-gray-600">{formatTimestamp(ev.created_at)}</span>
                            </div>
                            <div>
                              <span className="text-gray-500 block text-xs">Entity Type</span>
                              <span className="text-gray-600">{ev.entity_type}</span>
                            </div>
                          </div>
                          {ev.summary && (
                            <div className="mt-3 p-3 bg-surface rounded border border-gray-200">
                              <span className="text-gray-500 text-xs block mb-1">Summary</span>
                              <span className="text-gray-600 text-sm">{ev.summary}</span>
                            </div>
                          )}
                              ...
                              <div className="mt-3 p-3 rounded border border-purple-500/30 bg-purple-500/5">
                                ...
                                <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
                                  ...
                                  <div>
                                    ...
                                    <p className="text-gray-600 capitalize">{ctx.severity}</p>
                                  </div>
                                  <div>
                                    <span className="text-gray-500 text-xs">Predicted Risk</span>
                                    <p className="text-gray-600 capitalize">{ctx.predicted_risk_level || 'none'}</p>
                                  </div>
                                  <div>
                                    <span className="text-gray-500 text-xs">Confidence</span>
                                    <p className="text-gray-600 capitalize">{ctx.confidence_label}</p>
                                  </div>
                                </div>
                                {(ctx.explanation_summary || ctx.evidence_summary) && (
                                  <p className="text-xs text-gray-500 mt-2">{ctx.explanation_summary || ctx.evidence_summary}</p>
                                )}
                                <a href={`/ai-anomalies-v2?service=${encodeURIComponent(ev.entity_name)}${ctx.anomaly_id ? `&anomaly_id=${encodeURIComponent(ctx.anomaly_id)}` : ''}`} className="inline-flex items-center gap-1 mt-2 text-xs text-purple-400 hover:text-purple-300">
      ...
      {!isLoading && !error && events.length > 0 && (
        <div className="md:hidden space-y-3">
          {events.map((ev) => (
            <div key={ev.id}
              className="bg-surface border border-gray-200 rounded-lg p-4 space-y-3"
              onClick={() => setExpandedRow(expandedRow === ev.id ? null : ev.id)}
            >
              <div className="flex items-start justify-between">
                <div>
                  <h3 className="font-medium text-gray-900 text-sm">{ev.alert_name}</h3>
              ...
              <div className="flex items-center justify-between text-xs">
                <SeverityBadge severity={ev.severity} />
                <AiScoreBadge ctx={aiContextData?.contexts?.[ev.entity_name]} />
                <span className="text-gray-500">{timeAgo(ev.started_at)}</span>
