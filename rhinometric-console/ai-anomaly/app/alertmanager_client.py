...
class AlertmanagerClient:
    ...
    async def resolve_alert(
        ...
        """
        Resolve an alert in Alertmanager by sending it with endsAt = now.

        This tells AM the alert condition has ended. AM uses the labels
        to match the existing alert and mark it resolved.

        Args:
            alert_labels: The exact labels dict of the alert to resolve.
                          Must match the labels used when the alert was fired.
            annotations:  Optional annotations to attach to the resolution.

        Returns:
            True if resolution was sent successfully
        """
        if not self.enabled:
            logger.debug("Alertmanager disabled, skipping resolve")
            return False
        ...
        try:
            ...
            logger.info(f"Alert RESOLVED in Alertmanager: {alertname}")
            return True

        except httpx.HTTPError as e:
            logger.error(f"HTTP error resolving alert: {e}")
            return False
        except Exception as e:
