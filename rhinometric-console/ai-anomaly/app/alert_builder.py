...
Key features:
    ...
    - Build resolution payloads for Alertmanager (Phase 3)
...
# Phase 3: resolution counter
ALERTS_RESOLVED_TOTAL = Counter(
    'rhinometric_alerts_resolved_total',
    'Total number of alert resolutions sent to Alertmanager',
    ['channel', 'severity']
)
...
class AlertBuilder:
    ...
    def build_resolve_alert(
        ...
        """
        Build a resolution payload for Alertmanager (Phase 3).

        Alertmanager resolves an alert when it receives the SAME labels
        with endsAt set to the current time (or any time in the past).
        Labels must match exactly for AM to correlate and resolve.

        Args:
            metric_name:  Metric name (must match the fired alert)
            severity:     Severity (must match the fired alert)
            additional_labels: Extra labels that were on the original alert

        Returns:
            Resolution payload formatted for Alertmanager JSON API
        """
        # Generate fingerprint identically to build_alert
        fingerprint = self._generate_fingerprint(metric_name, severity, additional_labels)
        ...
        # Labels MUST match the fired alert exactly
        labels = {
            "alertname": f"AnomalyDetected_{metric_name}",
            "severity": severity,
            "metric": metric_name,
        ...
        resolve_payload = {
            "labels": labels,
            "annotations": {
                "summary": f"RESOLVED - {metric_name.replace('_', ' ').title()}",
                "resolved_at": now_utc
            },
            "endsAt": now_utc,
            "generatorURL": f"https://anomaly.rhinometric.com/anomalies?metric={metric_name}"
        }
        ...
        print(f"[AlertBuilder] Built RESOLVE payload for {metric_name} ({severity})")
