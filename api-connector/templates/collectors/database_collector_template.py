...
resource = Resource.create({
    "service.name": f"database-collector-{{ collector_name | default('default') }}",
...
DB_TYPE = os.getenv("DB_TYPE", "{{ db_type | trim }}")  # postgresql, mysql, mongodb
DB_HOST = os.getenv("DB_HOST", "{{ db_host | trim }}")
DB_PORT = int(os.getenv("DB_PORT", "{{ db_port | default(5432) }}"))
DB_NAME = os.getenv("DB_NAME", "{{ db_name | trim }}")
DB_USER = os.getenv("DB_USER", "{{ db_user | trim }}")
DB_PASSWORD = os.getenv("DB_PASSWORD", "{{ db_password | trim }}")
QUERY_SQL = os.getenv("QUERY_SQL", """{{ query_sql | trim }}""")
METRICS_PORT = int(os.getenv("METRICS_PORT", "{{ metrics_port | default(9300) }}"))
QUERY_INTERVAL = int(os.getenv("QUERY_INTERVAL", "{{ query_interval | default(60) }}"))

# Connection settings
CONNECTION_TIMEOUT = int(os.getenv("CONNECTION_TIMEOUT", "{{ connection_timeout | default(10) }}"))  # seconds
QUERY_TIMEOUT = int(os.getenv("QUERY_TIMEOUT", "{{ query_timeout | default(30) }}"))  # seconds
MAX_RETRIES = int(os.getenv("MAX_RETRIES", "{{ max_retries | default(3) }}"))
RETRY_DELAY = int(os.getenv("RETRY_DELAY", "{{ retry_delay | default(5) }}"))  # seconds
...
def create_db_engine():
    ...
    SQLAlchemyInstrumentor().instrument(
        engine=engine,
        service=f"database-collector-{{ collector_name | default('default') }}"
    )
