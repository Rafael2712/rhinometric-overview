...
resource = Resource.create({
    "service.name": f"mqtt-collector-{{ collector_name | default('default') }}",
...
MQTT_BROKER = "{{ broker_host }}"
MQTT_PORT = {{ broker_port | default(1883) }}
MQTT_USERNAME = "{{ username }}" if "{{ username }}" else None
MQTT_PASSWORD = "{{ password }}" if "{{ password }}" else None
MQTT_CLIENT_ID = "{{ client_id | default('rhinometric-collector') }}"
MQTT_TOPICS = {{ topics | tojson }}
