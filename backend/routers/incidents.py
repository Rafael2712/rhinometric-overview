...
# AI Incident Briefing (Phase 5.1)
from services.incident_briefing import generate_briefing
router = APIRouter()

VALID_STATUSES = {"open", "investigating", "resolved"}
ACTIVE_ALERT_STATUSES = {"firing", "acknowledged", "silenced", "escalated", "active"}
...
def check_incident_resolution(db: Session, incident_id) -> None:
    ...
    still_active = db.query(AlertEvent).filter(
        ...
        AlertEvent.status.in_(list(ACTIVE_ALERT_STATUSES)),
    ).count()
    if still_active == 0:
...
def _generate_incident_ai_decision(incident: Incident) -> Dict[str, Any]:
    ...
    return {
        ...
        "summary": f"Incident {incident.entity_name} classified as {decision}.",
        "reason": f"Based on severity={incident.severity} and status={incident.status}.",
        "evidence": [
            f"Incident severity: {incident.severity}",
            f"Incident status: {incident.status}",
        ],
        "recommended_actions": (
            ["Start immediate response and stakeholder escalation."]
            if decision == "escalate"
            else ["Continue active investigation and watch linked alerts."]
            if decision == "notify"
            else ["Continue monitoring for status or severity changes."]
            if decision == "monitor"
...
@router.get("/{incident_id}/ai-briefing")
async def get_ai_briefing(
    incident_id: str,
    db: Session = Depends(get_db),
    current_user: UserModel = Depends(get_current_user),
):
    """Return the stored AI briefing for an incident, or None if not yet generated."""
    import uuid as _uuid
    try:
        inc_uuid = _uuid.UUID(incident_id)
    except ValueError:
    ...
    if not incident:
        raise HTTPException(status_code=404, detail="Incident not found")
    return {"briefing": incident.ai_briefing}


@router.post("/{incident_id}/ai-briefing")
async def create_ai_briefing(
    incident_id: str,
    current_user: UserModel = Depends(require_role(["OWNER", "ADMIN", "OPERATOR"])),
    db: Session = Depends(get_db),
):
    """Generate (or regenerate) the AI briefing for an incident and persist it."""
    import uuid as _uuid
    try:
        inc_uuid = _uuid.UUID(incident_id)
    except ValueError:
    ...
    briefing = await generate_briefing(incident_id, db)
    if "error" in briefing:
        raise HTTPException(status_code=500, detail=briefing["error"])

    incident.ai_briefing = briefing
    incident.updated_at = datetime.now(timezone.utc)

    create_timeline_event(
        db, inc_uuid, "ai_analysis",
        description=f"AI Briefing generated by {current_user.username} (engine: {briefing.get('engine', 'unknown')})",
        created_by=current_user.username,
    )
    db.commit()
...
@router.get("/{incident_id}/ai-decision", response_model=AIDecisionResponse)
async def get_ai_decision(
    ...
    if not incident:
        raise HTTPException(status_code=404, detail="Incident not found")
    return {"ai_decision": incident.ai_decision}


@router.post("/{incident_id}/ai-decision", response_model=AIDecisionResponse)
async def create_ai_decision(
    ...
    incident.ai_decision = _generate_incident_ai_decision(incident)
    incident.updated_at = datetime.now(timezone.utc)
    create_timeline_event(
        db, inc_uuid, "ai_analysis",
        description=f"AI decision generated by {current_user.username}",
        created_by=current_user.username,
        metadata={"type": "ai_decision"},
    )
    db.commit()
    return {"ai_decision": incident.ai_decision}
