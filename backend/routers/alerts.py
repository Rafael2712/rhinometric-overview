    ...
    try:
        ...
        for alert in deduped:
            formatted_alerts.append(Alert(
                ...
                ai_decision=alert.get("ai_decision"),
            ))
...
def _generate_alert_ai_decision(ev) -> Dict[str, Any]:
    ...
    if status in ("resolved", "dismissed"):
        ...
        risk = "low"
        confidence = "high"
    elif severity == "critical" or status == "escalated":
        decision = "escalate"
        risk = "high"
        confidence = "high"
    ...
    return {
        ...
        "summary": f"Alert {ev.alert_name} classified as {decision}.",
        "reason": f"Based on severity={ev.severity} and status={ev.status}.",
        "evidence": [
            f"Alert severity: {ev.severity}",
            f"Alert status: {ev.status}",
            f"Entity: {ev.entity_name}",
        ],
        "recommended_actions": (
            ["Immediate response and escalation required."]
            if decision == "escalate"
            else ["Investigate and notify relevant operators."]
            if decision == "notify"
            else ["Observe behavior and re-check if conditions worsen."]
            if decision == "monitor"
...
@router.get("/{alert_id}/ai-decision", response_model=AIDecisionResponse)
async def get_alert_ai_decision(
    ...
    return {"ai_decision": ev.ai_decision}


@router.post("/{alert_id}/ai-decision", response_model=AIDecisionResponse)
async def create_alert_ai_decision(
    ...
    ev = _get_alert_event(db, alert_id)
    ev.ai_decision = _generate_alert_ai_decision(ev)
...
async def internal_ai_resolve(
    ...
    """Deprecated endpoint intentionally disabled: AI must not mutate alert lifecycle state."""
    raise HTTPException(
        status_code=410,
        detail="Deprecated endpoint: AI resolve is disabled. AI is advisory-only and cannot mutate lifecycle state.",
    )
