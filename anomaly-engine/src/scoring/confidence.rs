...
pub fn compute(snap: &ServiceSignalSnapshot, category_scores: &[f64; 4]) -> f64 {
...
/// If all 4 categories agree (all elevated or all zero), confidence is higher.
/// Mixed signals reduce confidence.
fn signal_agreement(scores: &[f64; 4]) -> f64 {
    ...
    match firing {
        4 => 0.95,  // All degraded — strong agreement
        3 => 0.85,
        2 => 0.70,
...
mod tests {
    ...
    fn test_immature_baseline() {
        ...
        let c = compute(&snap, &scores);
        assert!(c < 0.5, "Immature baseline should tank confidence: {c}");
    }
