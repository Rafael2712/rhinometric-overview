...
function getAiDecisionStyles(decision?: string | null): string {
  ...
  if (d === 'ignore') return 'bg-gray-500/10 text-gray-400 border-gray-500/30'
...
function AITriageBadge({ decision }: { decision?: AiTriageDecision | null }) {
  ...
  return (
    <div className="min-w-0">
      ...
      <p className="text-[10px] text-slate-500 dark:text-gray-400 mt-1 truncate">
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-2 sm:mb-6">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-slate-900 dark:text-white mb-1 sm:mb-2">Operational Alerts</h1>
          <p className="text-slate-500 dark:text-text-muted text-sm sm:text-base">Alerts requiring attention – generated when anomaly scores or predicted risk exceed operational thresholds</p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3 sm:gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <select
              ...
              className="bg-white dark:bg-surface-light border border-slate-300 dark:border-gray-700 text-slate-700 dark:text-white rounded px-2 sm:px-3 py-1.5 text-sm"
            >
              ...
              <table className="w-full table-fixed">
                <thead>
                  <tr className="border-b border-slate-200 dark:border-gray-700">
                    <th className="text-left py-2.5 px-3 text-xs font-semibold text-slate-500 dark:text-gray-400 uppercase w-[60px]">Status</th>
                    <th className="text-left py-2.5 px-3 text-xs font-semibold text-slate-500 dark:text-gray-400 uppercase" style={{width:'22%'}}>Alert</th>
                    <th className="text-left py-2.5 px-3 text-xs font-semibold text-slate-500 dark:text-gray-400 uppercase w-[88px]">Severity</th>
                    <th className="text-left py-2.5 px-3 text-xs font-semibold text-slate-500 dark:text-gray-400 uppercase w-[140px]">State</th>
                    <th className="text-left py-2.5 px-3 text-xs font-semibold text-slate-500 dark:text-gray-400 uppercase w-[52px]">AI</th>
                    <th className="text-left py-2.5 px-3 text-xs font-semibold text-slate-500 dark:text-gray-400 uppercase" style={{width:'12%'}}>Started</th>
                    <th className="text-left py-2.5 px-3 text-xs font-semibold text-slate-500 dark:text-gray-400 uppercase" style={{width:'26%'}}>Actions</th>
                  </tr>
                </thead>
                <tbody>
                  {filteredAlerts.map((alert) => {
                    return (
                      ...
                      >
                        ...
                        <td className="py-2.5 px-3">
                          <div className="min-w-0">
                            <p className="text-slate-900 dark:text-white text-sm font-medium break-words">{alert.labels.alertname}</p>
                            <p className="text-xs text-slate-500 dark:text-gray-400 line-clamp-1 break-all">
                  ...
                  >
                    <div className="flex items-start justify-between gap-2 mb-2">
                      <div className="flex items-center gap-2 min-w-0">
                        {getStatusIcon(alert.status)}
                        <span className="text-slate-900 dark:text-white font-medium text-sm truncate">{alert.labels.alertname}</span>
                      </div>
                    ...
                    <p className="text-xs text-slate-500 dark:text-gray-400 line-clamp-2 mb-2 ml-7">
                      {alert.annotations.summary || alert.annotations.description || 'No description'}
                    </p>
          ...
          <div className="bg-surface border border-gray-700 rounded-t-xl sm:rounded-lg shadow-2xl w-full sm:max-w-3xl max-h-[90vh] sm:max-h-[85vh] overflow-y-auto">
            <div className="sticky top-0 bg-surface z-10 flex items-center justify-between p-3 sm:p-4 border-b border-gray-700">
              <div className="min-w-0 flex-1 mr-3">
                <h2 className="text-lg sm:text-xl font-bold text-slate-900 dark:text-white truncate">{selectedAlert.labels.alertname}</h2>
                <p className="text-xs text-slate-500 dark:text-gray-400">Started: {new Date(selectedAlert.startsAt).toLocaleTimeString()}</p>
              </div>
              <button
            ...
            <div className="p-3 sm:p-4 space-y-3 sm:space-y-4">
              ...
              {selectedAlert.annotations.description && (
                <div className="card p-3 sm:p-4">
                  ...
                  <p className="text-slate-700 dark:text-gray-300 text-sm break-words">{selectedAlert.annotations.description}</p>
                </div>
              )}
              ...
              <div className="card p-3 sm:p-4">
                <h3 className="text-base sm:text-lg font-semibold text-slate-900 dark:text-white mb-3">Labels</h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3">
                  {Object.entries(selectedAlert.labels).map(([key, value]) => (
                    <div key={key} className="flex items-start gap-2 min-w-0">
                      <span className="text-slate-500 dark:text-gray-400 text-sm flex-shrink-0">{key}:</span>
              ...
              <div className="card p-3 sm:p-4 border-blue-500/20">
                <div className="flex items-center justify-between gap-2 mb-2">
                  <h3 className="text-base sm:text-lg font-semibold text-slate-900 dark:text-white">AI Triage</h3>
                ...
                {selectedAlert.ai_decision?.decision && (
                  <div className="space-y-2 text-sm">
                    <p><span className="text-slate-500 dark:text-gray-400">Summary:</span> <span className="text-slate-800 dark:text-gray-200">{formatAiField(selectedAlert.ai_decision.summary)}</span></p>
                    <p><span className="text-slate-500 dark:text-gray-400">Reason:</span> <span className="text-slate-800 dark:text-gray-200">{formatAiField(selectedAlert.ai_decision.reason)}</span></p>
                    <p><span className="text-slate-500 dark:text-gray-400">Noise:</span> <span className="text-slate-800 dark:text-gray-200">{formatAiField(selectedAlert.ai_decision.noise_assessment)}</span></p>
                    <div>
                      <p className="text-slate-500 dark:text-gray-400">Evidence:</p>
                      <ul className="list-disc pl-5 text-slate-800 dark:text-gray-200">
                    ...
                    <div>
                      <p className="text-slate-500 dark:text-gray-400">Recommended Actions:</p>
                      <ul className="list-disc pl-5 text-slate-800 dark:text-gray-200">
                  ...
                  {selectedAlert.incident_id ? (
                    <div className="flex items-center justify-between gap-3">
                      <div className="flex items-center gap-2 min-w-0">
                        ...
                        <div className="min-w-0">
                          <p className="text-sm font-medium text-slate-900 dark:text-white">Linked Incident</p>
                          <p className="text-xs text-slate-500 dark:text-gray-400 truncate">ID: {selectedAlert.incident_id}</p>
                        </div>
                      </div>
                      <button
                    ...
                    <div className="flex items-center justify-between gap-3">
                      <div className="flex items-center gap-2 min-w-0">
                        ...
                        <div className="min-w-0">
                          <p className="text-sm font-medium text-slate-700 dark:text-gray-300">No Incident Linked</p>
                          <p className="text-xs text-slate-500 dark:text-gray-500">Create an incident to track investigation and resolution</p>
                        </div>
                      </div>
                      <button
              ...
              {canAct(selectedAlert) ? (
                <div className="card p-3 sm:p-4 border-gray-600/50">
                  <h3 className="text-sm font-semibold text-slate-700 dark:text-gray-300 mb-3">Alert Actions</h3>
                  <div className="flex flex-col sm:flex-row gap-2 sm:gap-3">
                    ...
                    <div className="flex-1 flex gap-2">
                      <select
                        ...
                        className="bg-white dark:bg-surface-light border border-slate-300 dark:border-gray-700 text-slate-700 dark:text-white rounded px-2 py-1.5 text-sm w-20 flex-shrink-0"
