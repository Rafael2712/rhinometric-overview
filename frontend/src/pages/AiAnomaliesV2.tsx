    ...
    <div ref={cardRef} className={`rounded-lg border ${highlight ? 'ring-2 ring-indigo-400 ring-offset-1 ring-offset-white dark:ring-offset-gray-900' : ''} ${isHistory ? 'border-slate-200 dark:border-gray-700/40 opacity-75' : c.border} overflow-hidden bg-white dark:bg-surface transition-all duration-500`}>
            ...
            {anomaly.ai_decision?.decision ? (
              <div className="flex flex-col items-end">
                <div className="flex items-center gap-1">
                  ...
                  <span className="text-[10px] text-slate-500 dark:text-gray-400">· {formatAiField(anomaly.ai_decision.risk_level)} risk</span>
                </div>
                <p className="text-[10px] text-slate-500 dark:text-gray-400 max-w-[220px] truncate">
      ...
      {expanded && (
        <div className="px-4 pb-4 pt-3 border-t border-slate-200 dark:border-gray-700/40 space-y-4">
          ...
          <div className="p-3 border border-slate-200 dark:border-gray-700/40 rounded-lg">
            <div className="flex items-center justify-between gap-2 mb-2">
              <h4 className="text-[11px] text-slate-600 dark:text-gray-400 uppercase tracking-wider font-semibold">AI Triage Details</h4>
              {anomaly.ai_decision?.decision ? (
                <AITriageBadge decision={anomaly.ai_decision} />
              ) : (
            ...
            {anomaly.ai_decision?.decision ? (
              <div className="space-y-2 text-xs">
                <p><span className="text-slate-500 dark:text-gray-400">Summary:</span> <span className="text-slate-700 dark:text-gray-300">{formatAiField(anomaly.ai_decision.summary)}</span></p>
                <p><span className="text-slate-500 dark:text-gray-400">Reason:</span> <span className="text-slate-700 dark:text-gray-300">{formatAiField(anomaly.ai_decision.reason)}</span></p>
                <p><span className="text-slate-500 dark:text-gray-400">Noise:</span> <span className="text-slate-700 dark:text-gray-300">{formatAiField(anomaly.ai_decision.noise_assessment)}</span></p>
                <div>
                  <p className="text-slate-500 dark:text-gray-400">Evidence:</p>
                  <ul className="list-disc pl-5 text-slate-700 dark:text-gray-300">
            ...
            ) : <p className="text-xs text-slate-500 dark:text-gray-400">No AI triage decision yet.</p>}
          </div>
