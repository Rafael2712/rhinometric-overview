...
const STATUS_CONFIG: Record<SLOStatus, { bg: string; text: string; ring: string; icon: React.ReactNode; label: string }> = {
  healthy:  { bg: 'bg-green-50 dark:bg-green-900/30',  text: 'text-green-700 dark:text-green-400',  ring: 'ring-green-300 dark:ring-green-500/30',  icon: <CheckCircle2 className="w-3.5 h-3.5" />, label: 'Meeting SLO' },
  at_risk:  { bg: 'bg-amber-50 dark:bg-yellow-900/30', text: 'text-amber-700 dark:text-yellow-400', ring: 'ring-amber-300 dark:ring-yellow-500/30', icon: <AlertTriangle className="w-3.5 h-3.5" />, label: 'At Risk' },
  breached: { bg: 'bg-red-50 dark:bg-red-900/30',    text: 'text-red-700 dark:text-red-400',    ring: 'ring-red-300 dark:ring-red-500/30',    icon: <XCircle className="w-3.5 h-3.5" />, label: 'Breached' },
  no_data:  { bg: 'bg-slate-100 dark:bg-gray-800/50',   text: 'text-slate-500 dark:text-gray-500',   ring: 'ring-slate-200 dark:ring-gray-700/30',   icon: <Clock className="w-3.5 h-3.5" />, label: 'No Data' },
}
...
function SLICard({ title, icon, value, unit, targetLabel, budget, status, hint }: {
  ...
  return (
    <div className={`relative bg-white dark:bg-gray-800/50 border border-slate-200 dark:border-gray-700/50 rounded-xl p-5 ring-1 ${cfg.ring} transition-all hover:bg-slate-50 dark:hover:bg-gray-800/70`}>
      {/* Status indicator */}
      ...
      <div className="mb-3">
        <p className={`text-3xl font-bold ${hasData ? cfg.text : 'text-gray-600'}`}>
          {hasData ? value : '—'}
          {hasData && <span className="text-lg font-normal text-slate-400 dark:text-gray-500 ml-1">{unit}</span>}
        </p>
      ...
      <div className="flex items-center justify-between text-xs mb-2">
        <span className="text-slate-500 dark:text-gray-500">Target: <span className="text-slate-800 dark:text-gray-300">{targetLabel}</span></span>
      ...
      {budget !== null && budget !== undefined ? (
        <div>
          <div className="flex items-center justify-between text-xs mb-1">
            <span className="text-slate-500 dark:text-gray-500">Error budget</span>
        ...
        <div className="h-1.5 bg-slate-200 dark:bg-gray-700/30 rounded-full" />
      )}
    </div>
  )
}
...
function TypeBadge({ type }: { type: string }) {
  const color = type === 'http' ? 'text-blue-700 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/30 border border-blue-200 dark:border-transparent' : 'text-purple-700 dark:text-purple-400 bg-purple-50 dark:bg-purple-900/30 border border-purple-200 dark:border-transparent'
...
function DataSourceBadge({ monitoringMode, telemetryStatus }: { monitoringMode?: string; telemetryStatus?: string }) {
  if (monitoringMode !== 'telemetry_enabled') {
    return <span className="px-2 py-0.5 rounded text-xs font-medium text-amber-700 dark:text-amber-400 bg-amber-50 dark:bg-amber-900/30 border border-amber-200 dark:border-transparent">Synthetic</span>
  }
  const isActive = telemetryStatus === 'receiving_data' || telemetryStatus === 'connected'
  if (isActive) {
    return <span className="px-2 py-0.5 rounded text-xs font-medium text-green-700 dark:text-green-400 bg-green-50 dark:bg-green-900/30 border border-green-200 dark:border-transparent">Telemetry + Synthetic</span>
  }
  return <span className="px-2 py-0.5 rounded text-xs font-medium text-blue-700 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/30 border border-blue-200 dark:border-transparent">Telemetry (pending)</span>
}
...
function BudgetBar({ pct }: { pct: number | null }) {
  ...
  return (
    <div className="flex items-center gap-2">
      <div className="flex-1 h-2 bg-slate-200 dark:bg-gray-700 rounded-full overflow-hidden max-w-[100px]">
        <div className={`h-full rounded-full ${color}`} style={{ width: `${Math.min(pct, 100)}%` }} />
      </div>
      <span className="text-xs text-slate-600 dark:text-gray-400 w-12 text-right">{pct.toFixed(1)}%</span>
    </div>
  )
}
...
function ServiceDetailPanel({ serviceId, token, timeRange }: { serviceId: number; token: string; timeRange: string }) {
  ...
  return (
    <div className="p-6 bg-slate-50 dark:bg-gray-900/50 border-t border-slate-200 dark:border-gray-700/50 space-y-6">
      {/* SLI cards row */}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Availability trend */}
        <div>
          <h4 className="text-sm font-semibold text-slate-700 dark:text-gray-300 mb-2">Availability Trend (hourly)</h4>
          <div className="max-h-48 overflow-y-auto space-y-1 text-xs">
            {trend.length === 0 && <p className="text-slate-500 dark:text-gray-500">No trend data</p>}
            {trend.slice(-12).map((b: { time: string; availability_pct: number; avg_latency_ms: number }) => (
              <div key={b.time} className="flex justify-between text-slate-600 dark:text-gray-400">
        ...
        {/* Recent alerts */}
        <div>
          <h4 className="text-sm font-semibold text-slate-700 dark:text-gray-300 mb-2">Recent Alerts ({alerts.length})</h4>
          {alerts.length === 0 && <p className="text-xs text-slate-500 dark:text-gray-500">None in this window</p>}
          {alerts.slice(0, 5).map((a: { id: string; alert_name: string; severity: string; status: string }) => (
            <div key={a.id} className="flex items-center gap-2 text-xs text-slate-600 dark:text-gray-400 mb-1">
              <span className={`w-2 h-2 rounded-full ${a.status === 'firing' ? 'bg-red-500' : 'bg-green-500'}`} />
              <span className="truncate">{a.alert_name}</span>
              <span className="text-slate-500 dark:text-gray-600">{a.severity}</span>
            </div>
          ))}
        </div>
        ...
        {/* Recent incidents */}
        <div>
          <h4 className="text-sm font-semibold text-slate-700 dark:text-gray-300 mb-2">Recent Incidents ({incidents.length})</h4>
          ...
          {incidents.slice(0, 5).map((inc: { id: string; status: string; severity: string; started_at: string }) => (
            <div key={inc.id} className="flex items-center gap-2 text-xs text-slate-600 dark:text-gray-400 mb-1">
              <StatusBadge status={inc.status === 'resolved' ? 'healthy' : 'breached'} />
              <span className="text-slate-500 dark:text-gray-600">{inc.severity}</span>
            </div>
          ))}
        </div>
      </div>
...
function ServiceRow({ svc, token, timeRange }: { svc: any; token: string; timeRange: string }) {
  ...
  return (
    <>
      <tr
        className="border-b border-slate-200 dark:border-gray-800 hover:bg-slate-50 dark:hover:bg-gray-800/30 cursor-pointer transition-colors"
      ...
      >
        ...
        <td className="px-4 py-3 text-sm font-medium text-slate-900 dark:text-white">{svc.service_name}</td>
...
function MobileServiceCard({ svc, token, timeRange }: { svc: any; token: string; timeRange: string }) {
  ...
  return (
    <div className="bg-white dark:bg-gray-800/50 border border-slate-200 dark:border-gray-700/50 rounded-xl p-4 space-y-3">
      <div className="flex items-center justify-between">
        <div>
          <p className="text-slate-900 dark:text-white font-medium">{svc.service_name}</p>
      ...
      <button onClick={() => setExpanded(!expanded)} className="text-xs text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300">
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <select
          ...
          className="bg-white dark:bg-gray-800 border border-slate-300 dark:border-gray-700 rounded-lg px-3 py-2 text-sm text-slate-700 dark:text-gray-300 focus:ring-2 focus:ring-blue-500 focus:border-transparent"
        >
      ...
      {/* Service count stats */}
      <div className="flex items-center justify-between bg-white dark:bg-gray-800/30 border border-slate-200 dark:border-gray-700/50 rounded-xl px-6 py-4">
        <div className="flex items-center gap-6">
          <CompactStat label="Services" value={summary.total} color="text-slate-900 dark:text-white" />
      ...
      {!isLoading && services.length > 0 && (
        <div className="hidden md:block bg-white dark:bg-gray-800/30 border border-slate-200 dark:border-gray-700/50 rounded-xl overflow-hidden">
          <table className="w-full">
            <thead>
              <tr className="border-b border-slate-200 dark:border-gray-700 text-left text-xs text-slate-500 dark:text-gray-400 uppercase tracking-wider">
      ...
      {/* Empty state */}
      {!isLoading && services.length === 0 && (
        <div className="text-center py-16 bg-white dark:bg-gray-800/20 border border-slate-200 dark:border-gray-700/30 rounded-xl">
          <Shield className="w-14 h-14 text-gray-600 mx-auto mb-4" />
          <p className="text-slate-800 dark:text-gray-300 text-lg font-medium">No services monitored yet</p>
